...
; :Returns:
;    Three data files:
;    1. PAH fluxes, fractional contributions of templates, and PAH extinction
;    2. PAH equivalent widths
;    3. Blackbody properties and effective 9.7um optical depth
;
; :Params:
...
;      2015oct27, DSNR, modified for new directory structure; fixed bug
;                       in wavelength range specification
...
;
;-
pro ideos_pah,datafile,outdir=outdir
...
;========
  ...
  datadir = rootdir+'docs/datafiles/'
  
; PAH templates
  readcol,datadir+'smith_nftemp3.pahpars.ext.dat',$
          pp1wl,pp1fwhm,pp1peak,pp1flux,/silent
  readcol,datadir+'smith_nftemp4.pahpars.next.dat',$
          pp2wl,pp2fwhm,pp2peak,pp2flux,/silent
; Extinction curves
  readcol,datadir+'extinction_ice+hc.dat',icewl,icetau,/silent,skipline=1
  readcol,datadir+'extinction_chiar06.dat',$
          ec0wl_chiar,ec0arat_chiar,/silent,skipline=1
  readcol,datadir+'extinction_chiar06+f08572.dat',$
          ec0wl_i0857,ec0arat_i0857,/silent,skipline=1
; normalize ice tau profile to 1 at peak
  icetau=icetau/max(icetau)
; Galaxy fit results tables
  readcol,datafile,id1,bestfit,$
          bbf1av,bbf1tau,bbf1temp,bbf1norm,$
          bbf2av,bbf2tau,bbf2temp,bbf2norm,$
          bbf3av,bbf3tau,bbf3temp,bbf3norm,$
          bbf4av,bbf4tau,bbf4temp,bbf4norm,$
          pf1av,pf1tau,pf1norm,$
          pf2av,pf2tau,pf2norm,$
          fitlo,fithi,$
          format='(A,D,D,D,D,D,D,D,D,D,D,D,D,D,D,D,D,D,D,D,D,D,D)',/silent,$
          nlines=ngal
...
;========
  ...
  openw,outlun1,outdir+'ideos_pahfluxes.txt',/get_lun
  openw,outlun2,outdir+'ideos_pahweq.txt',/get_lun
  openw,outlun3,outdir+'ideos_tau.txt',/get_lun

  printf,outlun1,'# Col. 2: Best fit (1=Chiar+06,2=Chiar+06/F08572,3=either)'
  printf,outlun1,'# Col. 3-9: PAH fluxes
  printf,outlun1,'#    Fluxes are logarithmic, with linear units 10^-26 W m^-2'
  printf,outlun1,'# Col. 10-11: Fractional contribution of PAH templates'
  printf,outlun1,'# Col. 12-13: Visual extinction of PAH templates'
  printf,outlun1,'# IDEOS_ID','BF','6.2um','7.7um','8.6um',$
         '11.3um','12.7um','17um',$
         'Total','T1frac','T2frac','T1A_V','T2A_V',$
         format='(A-12,A-3,11A10)'

  printf,outlun2,'# Col. 2: Best fit (1=Chiar+06,2=Chiar+06/F08572,3=either)'
  printf,outlun2,'# Col. 3-5: PAH eq. widths, observed-frame'
  printf,outlun2,'#    w.r.t. extincted continuum
  printf,outlun2,'# Col. 6-8: PAH eq. widths, observed-frame'
  printf,outlun2,'#    w.r.t. extincted continuum corrected for ice absorption
  printf,outlun2,'#    Eq. widths are logarithmic, with linear units microns'
  printf,outlun2,'# IDEOS_ID','BF','6.2um','7.7um','11.3um',$
         '6.2um','7.7um','11.3um',$
         format='(A-12,A-3,6A10)'

  printf,outlun3,'# Col. 2: Best fit (1=Chiar+06,2=Chiar+06/F08572,3=either)'
  printf,outlun3,'# Col. 3: Effective 9.7um silicate optical depth
  printf,outlun3,'# Col. 4-7: Blackbody visual extinctions (-1 = no flux)
  printf,outlun3,'# Col. 8-11: Blackbody temperatures (0 = no flux)
  printf,outlun3,'# IDEOS_ID','BF','tau9.7',$
                 'BB1A_V','BB2A_V','BB3A_V','BB4A_V',$
                 'BB1T','BB2T','BB3T','BB4T',$
                 format='(A-12,A-3,5A8,4A6)'
...
;  ecarat_chiar_int=interpol(ec0arat_chiar,ec0wl_chiar,wlcommon)
;  ecarat_i0857_int=interpol(ec0arat_i0857,ec0wl_i0857,wlcommon)
;  icetau_int=interpol(icetau,icewl,wlcommon)
  status_rb=$
     sap_rebin(ec0wl_chiar,ec0arat_chiar,dblarr(n_elements(ec0wl_chiar)),$
               ecwl_chiar_int,ecarat_chiar_int,ecerr_chiar_int,ref=wlcommon)
  status_rb=$
     sap_rebin(ec0wl_i0857,ec0arat_i0857,dblarr(n_elements(ec0wl_i0857)),$
               ecwl_i0857_int,ecarat_i0857_int,ecerr_i0857_int,ref=wlcommon)
  status_rb=$
     sap_rebin(icewl,icetau,dblarr(n_elements(icewl)),$
               icewl_int,icetau_int,iceerr_int,ref=wlcommon)
...
;
  for i=0,ngal-1 do begin
     ...
     if bestfit[i] eq 1 then ecarat_int = ecarat_chiar_int $
     else ecarat_int = ecarat_i0857_int
;
...
;    blackbodies
     ...
     bbs4flux_ext_noice=dblarr(n_elements(wlcommon))
...
;       Unextincted and extincted blackbody spectra, arbitrary normalization
        ...
        bbs4flux[j]=1/wlcommon[j]^3/(exp(14387.87/wlcommon[j]/bbf4temp[i])-1)
        ...
        bbs4flux_ext_noice[j]=bbs4flux[j]*exp(-bbf4av[i]*ecarat_int[j]/1.086)
        ...
        bbs4flux_ext[j]=bbs4flux_ext_noice[j]*exp(-bbf4tau[i]*icetau_int[j])
...
;     ps2flux_max=max(ps2flux,ps2ind_max)
     ...
     bbs4flux_max=max(bbs4flux,bbs4ind_max)
...
;    accordingly.
     ...
     if (bbs4ind_max lt fitloind) then bbs4ind_max=fitloind
     if (bbs4ind_max gt fithiind) then bbs4ind_max=fithiind
     bbs1flux_max=bbs1flux[bbs1ind_max]
     bbs2flux_max=bbs2flux[bbs2ind_max]
     bbs3flux_max=bbs3flux[bbs3ind_max]
     bbs4flux_max=bbs4flux[bbs4ind_max]
...
; Factor 10^26 converts template fluxes from W/m^2/Hz to 10^-26 W/m^2/Hz (1 Jy).
...
;    Normalizations
     ...
     bb4fluxrat_max=bbf4norm[i]/bbs4flux_max
;    Spectra
     ...
     bb4_ext_noice = bbs4flux_ext_noice*bb4fluxrat_max
     bb1_ext = bbs1flux_ext*bb1fluxrat_max
     bb2_ext = bbs2flux_ext*bb2fluxrat_max
     bb3_ext = bbs3flux_ext*bb3fluxrat_max
     bb4_ext = bbs4flux_ext*bb4fluxrat_max
     bb1 = bbs1flux*bb1fluxrat_max
     bb2 = bbs2flux*bb2fluxrat_max
     bb3 = bbs3flux*bb3fluxrat_max
     bb4 = bbs4flux*bb4fluxrat_max
     bb_ext_noice = bb1_ext_noice + bb2_ext_noice + bb3_ext_noice + bb4_ext_noice
     bb_ext = bb1_ext + bb2_ext + bb3_ext + bb4_ext
     bb = bb1 + bb2 + bb3 + bb4
...
;    Fluxes
     if (pf1norm[i] eq 0. AND pf2norm[i] eq 0.) then begin
        ...
        p6=alog10(pf1norm[i]*p1f6  +pf2norm[i]*p2f6 )-6.+26.
        p7=alog10(pf1norm[i]*p1f7  +pf2norm[i]*p2f7 )-6.+26.
        p8=alog10(pf1norm[i]*p1f8  +pf2norm[i]*p2f8 )-6.+26.
        p11=alog10(pf1norm[i]*p1f11 +pf2norm[i]*p2f11)-6.+26.
        p12=alog10(pf1norm[i]*p1f12 +pf2norm[i]*p2f12)-6.+26.
        p17=alog10(pf1norm[i]*p1f17 +pf2norm[i]*p2f17)-6.+26.
        ptot=alog10(pf1norm[i]*p1ftot+pf2norm[i]*p2ftot )-6.+26.
        p1frac=pf1norm[i]*p1ftot / (pf1norm[i]*p1ftot+pf2norm[i]*p2ftot)
        p2frac=pf2norm[i]*p2ftot / (pf1norm[i]*p1ftot+pf2norm[i]*p2ftot)
;
     endelse
;    Spectra
;     p1 = ps1flux*pf1norm[i]
;     p2 = ps2flux*pf2norm[i]
...
;    Blackbody flux densities (10^-26 W/m^2 per micron)
     ...
     bb7 =alog10(bb[w7ind])+alog10(2.99792)+14-2*alog10(7.70)
     ...
     bb7_ext =alog10(bb_ext[w7ind])+alog10(2.99792)+14-2*alog10(7.70)
     ...
     bb7_ext_noice =alog10(bb_ext_noice[w7ind])+alog10(2.99792)+14-2*alog10(7.70)
     bb8_ext_noice =alog10(bb_ext_noice[w8ind])+alog10(2.99792)+14-2*alog10(7.90)
     bb11_ext_noice =alog10(bb_ext_noice[w11ind])+alog10(2.99792)+14-2*alog10(11.2)
;    Integrated optical depth at 9.7 um
     bbsiltau=-alog(bb_ext[wsilind]/bb[wsilind])
;    Maximum value of A_V
     bbavmax=max([bbf1av[i],bbf2av[i],bbf3av[i],bbf4av[i]])
...
;
;    Sort BB temperatures and extinctions in order of descending temperature
;
     bbnorms = [bbf1norm[i],bbf2norm[i],bbf3norm[i],bbf4norm[i]]
     bbtemp = [bbf1temp[i],bbf2temp[i],bbf3temp[i],bbf4temp[i]]
     bbav = [bbf1av[i],bbf2av[i],bbf3av[i],bbf4av[i]]
     inonzero = where(bbnorms ne 0d,ctnonzero)
     ctzero = 4 - ctnonzero
;    Set T=0, A_V=-1 if component zeroed out
     if ctzero gt 0 then begin
        bbtemp[0:ctnonzero-1] = bbtemp[inonzero]
        bbtemp[ctnonzero:3] = 0d
        bbav[0:ctnonzero-1] = bbav[inonzero]
        bbav[ctnonzero:3] = -1d
     endif
     bbtemp_i = reverse(sort(bbtemp))
...
;    PAH equivalent widths [observed frame]
     ...
     p7weq_noice = bad
     p11weq_noice = bad
     if p6 ne bad then begin
        p6weq = p6 - bb6_ext
        p7weq = p7 - bb8_ext
        p11weq = p11 - bb11_ext
        p6weq_noice = p6 - bb6_ext_noice
        p7weq_noice = p7 - bb8_ext_noice
        p11weq_noice = p11 - bb11_ext_noice
     endif
     
;    Print to output file
     printf,outlun1,id1[i],bestfit[i],p6,p7,p8,p11,p12,p17,$
            ptot,p1frac,p2frac,pf1av[i],pf2av[i],$
            format='(A-12,I3,11D10.4)'

     printf,outlun2,id1[i],bestfit[i],p6weq,p7weq,p11weq,$
            p6weq_noice,p7weq_noice,p11weq_noice,$
            format='(A-12,I3,6D10.4)'

     printf,outlun3,id1[i],bestfit[i],bbsiltau,$
            bbav[0],bbav[1],bbav[2],bbav[3],$
            bbtemp[0],bbtemp[1],bbtemp[2],bbtemp[3],$
            format='(A-12,I3,D8.4,4D8.2,4I6)'
